...
export default function Header({
  onSignIn, location, userName, isLogged, logOut,
}) {
  ...
  return (
    <header className={location.pathname === '/' ? `header ${navIsOpen}` : `header__alternative ${navIsOpen}`}>
        ...
        <Navigation onSignIn={onSignIn} location={location} openNav={navIsOpen} userName={userName} isLogged={isLogged} logOut={logOut} />
    </header>
  );
}
